...
`users`, `groups`, `group_members`, `expenses`, `expense_shares`, `settlements`.
Tum PK'lar `uuid` ve `gen_random_uuid()` ile DB tarafinda uretilir (pgcrypto).
...
1 admin + 3 kullanici, 1 grup (`Ev Arkadaslari`), 4 uyelik, 3 harcama ve 11 pay.
Tum seed kullanicilarinin sifresi `Password123!` (bcrypt ile hashlenir).
ID'ler sabit UUID oldugundan seed tekrar calistirilabilir.
...
`.env.example` dosyasina bakin: `NODE_ENV`, `PORT`, `LOG_LEVEL`, `DATABASE_URL`,
`DATABASE_SSL`, `JWT_SECRET`.
